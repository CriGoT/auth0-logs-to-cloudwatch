...
const project = require('../../node_modules/auth0-log-extensions-ui/package.json');
console.info('Running production configuration...');

const config = require('./config.base.js');
const uiVersion = project.version.replace(/.\d+$/g, '');
config.profile = false;

// Build output, which includes the hash.
config.output.hash = true;
config.output.filename = 'auth0-log-extensions.ui.' + uiVersion + '.js';
...
config.plugins = config.plugins.concat([
  ...
  new ExtractTextPlugin('auth0-log-extensions.ui.' + uiVersion + '.css', {
    allChunks: true
  }),

  // Separate the vender in a different file.
  new webpack.optimize.CommonsChunkPlugin('vendors', 'auth0-log-extensions.ui.vendors.' + uiVersion + '.js'),
